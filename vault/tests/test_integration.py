...
from datadog_checks.vault.metrics import METRIC_MAP
...
def test_integration(aggregator, check, instance):
    ...
    assert_collection(aggregator, instance['tags'])
...
def test_e2e(dd_agent_check, e2e_instance):
    ...
    assert_collection(aggregator, e2e_instance['tags'], runs=2)


def assert_collection(aggregator, tags, runs=1):
    metrics = set(METRIC_MAP.values())
    ...
    aggregator.assert_metric('vault.is_leader', value=1, tags=['instance:foobar', 'is_leader:true'], count=runs)
