...
def test_description_default():
    ...
    assert spec.data['files'][0]['options'][0]['description'] == (
...
def test_multiple_default_instances():
    spec = get_spec(
        """
        name: foo
        version: 0.0.0
    ...
    assert spec.data['files'][0]['options'][0]['multiple'] is True


def test_multiple_default_instances_nested():
    spec = get_spec(
        """
        name: foo
        version: 0.0.0
    ...
    assert not spec.errors
    assert spec.data['files'][0]['options'][0]['options'][0]['multiple'] is False
