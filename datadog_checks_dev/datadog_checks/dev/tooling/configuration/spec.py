...
from .constants import KNOWN_SECTION_DESCRIPTIONS
...
def options_validator(options, loader, file_name, *sections):
    ...
    for option_index, option in enumerate(options, 1):
        ...
        if 'template' in option:
            parameters = option.copy()
            parameters.pop('template')

            try:
                template = loader.templates.load(option['template'], parameters)
        ...
        if not sections and option_name in KNOWN_SECTION_DESCRIPTIONS:
            option.setdefault('description', KNOWN_SECTION_DESCRIPTIONS[option_name])
        ...
        if 'value' in option:
            ...
            default_multiple = not sections and option_name == 'instances'
            option.setdefault('multiple', default_multiple)
